...
void gr_rle_expand_scanline(ubyte *dest, ubyte *src, int x1, int x2, int dwidth)
{
	int i = 0;
	ubyte count;
	ubyte color=0;
	...
	if ( x1+count > x2 )	{
		count = x2-x1+1;
		rle_stosb(dest, count<<dwidth, color );
		return;
	}
...
int gr_bitmap_rle_compress( grs_bitmap * bmp )
{
	...
	rle_data=(ubyte *)mymalloc( (bmp->bm_w+1)* bmp->bm_h );
