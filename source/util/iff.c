...
//  The order was s[3] ... s[0]  MWA
	...
	x.s[0] = f->data[f->position++];
	if (f->position>=f->length) return EOF;
	x.s[1] = f->data[f->position++];
	if (f->position>=f->length) return EOF;
	x.s[2] = f->data[f->position++];
	if (f->position>=f->length) return EOF;
	x.s[3] = f->data[f->position++];
...
int open_fake_file(char *ifilename,FFILE *ffile)
{
	...
	if ((ifile = cfopen(ifilename,"rb")) == NULL)
		return IFF_NO_FILE;
	...
	for (n=1;n<len;n++) {
		...
		if (src[n] == last) {
			...
			if (rep_cnt > 2 || lit_cnt < 2) {
				...
				if (lit_cnt > 1) { //save old lit count
					*cnt_ptr = lit_cnt-2;
					--dptr;
				}
