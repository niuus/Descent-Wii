			...
			case OP_FLATPOLY: {
				...
				if (g3_check_normal_facing(vp(p+4),vp(p+16)) > 0) {
					...
					if (w(p+28) >= 0)
						...
						color = interp_color_table[w(p+28)&0x00FF].pal_entry;
					}
					else {
						color = glow_values[glow_num];
						glow_num = -1;
						if (color == -1) {
							color = MIN(32, MAX(0, f2i(model_light*32)));
							color = gr_fade_table[interp_color_table[color].pal_entry];
						}
						else if (color == -2)
							color = 255; // hack
					}
		...
		switch (w(p)) {
			...
			case OP_FLATPOLY: {
				...
				if (w(p+28)>=0)
					...
					gr_setcolor(interp_color_table[w(p+28)&0x00FF].pal_entry);
...
static int find_color_index(short rgb15color)
{
	int i;
	for (i=0; i < n_interp_colors; i++) {
		if (interp_color_table[i].rgb15 == rgb15color)
			return i;
	}

	interp_color_table[i].rgb15 = rgb15color;
	interp_color_table[i].pal_entry = (short)gr_find_closest_color_15bpp(rgb15color);
	n_interp_colors++;
...
static void init_model_sub(ubyte *p, int uninitialize)
{
	...
	while (w(p) != OP_EOF) {
		...
		switch (w(p)) {
			...
			case OP_FLATPOLY: {
				...
				if (!uninitialize)
					w(p+28) = find_color_index(w(p+28))|0x8000;
				else
					w(p+28) = interp_color_table[w(p+28)&0x00FF].rgb15;
			...
			case OP_SORTNORM:
				...
				init_model_sub(p+w(p+28), uninitialize);
				init_model_sub(p+w(p+30), uninitialize);
				p += 32;
			...
			case OP_SUBCALL: {
				init_model_sub(p+w(p+16), uninitialize);
				p += 20;
				break;
...
void g3_init_polygon_model(void *model_ptr)
{
	...
	init_model_sub((ubyte *) model_ptr, 0);
}

void g3_uninit_polygon_model(void *model_ptr)
{
	highest_texture_num = -1;
	init_model_sub((ubyte*)model_ptr, 1);
}

void g3_remap_interp_colors(void)
{
	int index;
	for (index=0; index < n_interp_colors; index++) {
		interp_color_table[index].pal_entry = gr_find_closest_color_15bpp(interp_color_table[index].rgb15);
	}
}
