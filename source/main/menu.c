...
try_again:
		...
		Newmenu_allowed_chars = Newmenu_number_chars;
...
void do_option ( int select)
{
	switch (select) {
		...
		case MENU_DEMO_PLAY:
			{
				char demo_file[16];
				if (newmenu_get_filename( TXT_SELECT_DEMO, "Demos", demo_file, 1 ))	{
...
static void do_cockpit_menu(void)
{
	int i;
	newmenu_item m[3];
	int i1 = 0;
	...
	m[0].text = "STATUS BAR";
	m[1].text = "FULL COCKPIT";
	m[2].text = "FULL SCREEN";

	for (i=0; i < 3; i++) {
		m[i].type = NM_TYPE_RADIO; m[i].value = 0; m[i].group = 0;
	}
	...
	do {
		i1 = newmenu_do4( NULL, "Cockpit Mode", 3, m, NULL, i1, NULL, -1, -1, 1 );
...
//		m[2].type = NM_TYPE_CHECK; m[2].text=TXT_REVERSE_STEREO; m[2].value=Config_channels_reversed;
		...
		m[5].type = NM_TYPE_MENU; m[5].text="Cockpit Mode...";
...
#ifdef NETWORK
	char *net_error = network_get_status();

	if (net_error) {
		nm_messagebox( NULL, 1, TXT_OK, net_error);
		return;
	}
#endif
