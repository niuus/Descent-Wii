...
#define XYZ(v) (uint)((v)->x),(uint)((v)->y),(uint)((v)->z)
...
//*******					mprintf((0,"Obj %d moved backwards\n",obj-Objects));
				...
				if (sim_time < 0 || sim_time>old_sim_time) {
					#ifndef NDEBUG
					mprintf((0,"Bogus sim_time = %x, old = %x\n",(uint)sim_time,(uint)old_sim_time));
					if (obj == debug_obj)
						printf("   Bogus sim_time = %x, old = %x, attempted_dist = %x, actual_dist = %x\n",(uint)sim_time,(uint)old_sim_time,(uint)attempted_dist,(uint)actual_dist);
					//Int3(); Removed by Rob
					#endif
				...
				if ( !(obj->flags&OF_SHOULD_BE_DEAD) )	{
					...
					if (obj->mtype.phys_info.flags & PF_STICK) {		//stop moving
						...
						if (obj == debug_obj) {
							...
							printf("   wall_part %x, new velocity = %x %x %x\n",(uint)wall_part,XYZ(&obj->mtype.phys_info.velocity));
						}
						#endif
