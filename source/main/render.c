...
void render_side(segment *segp, int sidenum)
{
	...
	if (sidep->type == SIDE_IS_QUAD) {
		if (v_dot_n0 >= 0) {
			render_face(segp-Segments, sidenum, 4, vertnum_list, sidep->tmap_num, sidep->tmap_num2, sidep->uvls, normals);
		}
	} else {
...
{
	const sort_item *a = (const sort_item*)_a;
	const sort_item *b = (const sort_item*)_b;
