...
void draw_automap()
{
	...
	gr_bm_ubitblt( Page.cv_bitmap.bm_w, Page.cv_bitmap.bm_h, Page.cv_bitmap.bm_x, Page.cv_bitmap.bm_y, 0, 0, &Page.cv_bitmap, &VR_screen_page.cv_bitmap );
	bitblt_to_screen();
}

#define LEAVE_TIME F0_5
...
#if 0 // exclude this for now, we may want it later
static void modex_print_message(int x, int y, char *str)
{
...
void do_automap( int key_code )	{
	...
	gr_init_sub_canvas(&Page,grd_curcanv,0,0,320,400);
	gr_init_sub_canvas(&DrawingPage,&Page,16,69,288,272);
	...
	if (Automap_background.bm_data)
		myfree(Automap_background.bm_data);
	Automap_background.bm_data = NULL;
	...
	while(!done)	{
		...
		if ( leave_mode==0 && Controls.automap_state && (timer_get_fixed_seconds()-entry_time)>LEAVE_TIME) {
			fprintf(stderr, "leave_mode=1\n");
			leave_mode = 1;
		}

		if ( !Controls.automap_state && (leave_mode==1) ) {
			fprintf(stderr, "automap done\n");
			done=1;
		}
