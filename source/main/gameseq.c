...
char playername_allowed_chars[] = "AZ09__--";
...
do_menu_again:
	...
	#ifndef APPLE_OEM
	if (!newmenu_get_filename( TXT_SELECT_PILOT, "Players", filename, allow_abort_flag ))	{
		return 0;		// They hit Esc in file selector
	}
	#else
	newmenu_get_filename( "Select Pilot", "Players", filename, 0 );		// no abort allowed ever -- and change title of menubox
	#endif
...
static void mac_do_checksum_calc(void *_b, int len, unsigned int *s1, unsigned int *s2)
{
	ubyte *b = (ubyte*)_b;
	while(len--)	{
		*s1 += *b++;
