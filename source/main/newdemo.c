...
static void nd_read_byte(void *b)
{
	newdemo_read((byte*)b, 1, 1);
}
...
#ifndef NETWORK
	if (Newdemo_game_mode & (GM_MULTI|GM_TEAM)) {
...
char demoname_allowed_chars[] = "AZ09__--  ";
...
try_again:
	...
	if (exit == -2) {					// got bumped out from network menu
		...
		if (filename[0] != '\0') {
			//strcpy(save_file, "/Descent/");
			strcpy(save_file, filename);
			strcat(save_file, ".dem");
		} else
			//sprintf (save_file, "/Descent/tmp%d.dem", tmpcnt++);
...
void newdemo_start_playback(char * filename)
{
	...
	if (filename==NULL) {
		...
		while ((entry = readdir(dir))) {
			char *t;
			if ((t = strchr(entry->d_name,'.')) == NULL)
				continue;
			if (stricmp(t, ".dem"))
				continue;
			mprintf((0, "Found a demo file, %s\n", entry->d_name));
			NumFiles++;
		}
		closedir(dir);
