...
ubyte object_buffer[IPX_MAX_DATA_SIZE];		// used for tmp netgame packets as well as sending object data

char *net_not_ready = "Network has not initialized\nWait and try again";
...
network_init(void)
{
	// So you want to play a netgame, eh?  Let's get a few things straight
...
network_new_player(netplayer_info *their)
{
	...
	if (Network_game_type == IPX_GAME) {
		const int zero = 0;
		if (memcmp(their->network_info.ipx_info.server, &zero, 4))
...
//MWA		*(short *)(object_buffer+loc) = -1;		loc += 2;
		...
		for (i = Network_send_objnum; i <= Highest_object_index; i++)
		{
			...
			if ( ((IPX_MAX_DATA_SIZE-1) - loc) < (SIZEOF_OBJECT+5) )
...
//MWA		memcpy(object_buffer+loc, &Objects[i], sizeof(object));	loc += sizeof(object);
			...
			memcpy(&(object_buffer[loc]), &tmps, 2);						loc += 2;
			if (Network_game_type == IPX_GAME) {
				pack_object(object_buffer+loc, Objects+i);                  loc += SIZEOF_OBJECT;
			}
...
void network_process_gameinfo(ubyte *data)
{
	...
	// this takes care of byte-swapping and struct padding
	if ( Network_game_type == IPX_GAME )
...
//	if ( length < sizeof(sequence_packet) ) return;
	...
	case PID_GAME_INFO:
		...
		// sizeof(netgame_info) no longer matches the length on the wire due to struct padding
		//if (length != sizeof(netgame_info))
		//	mprintf((0, " Invalid size %d for netgame packet. Should be %d\n", length, sizeof(netgame_info)));
...
network_read_object_packet( ubyte *data )
{
	...
	for (i = 0; i < nobj; i++)
	{
		...
		{
			Assert(frame_num == remote_frame_num);
			...
			if (objnum != -1) {
				...
				unpack_object(obj, data+loc);              loc += SIZEOF_OBJECT;
				segnum = obj->segnum;
...
void network_game_param_poll( int nitems, newmenu_item * menus, int * key, int citem )
{
	...
	return;
#else
 	if (menus[5].value && !menus[9].value) {
		menus[9].value = 1;
		menus[9].redraw = 1;
	}
	if (menus[7].value) {
...
{
	int i;
	int opt, opt_mode, opt_closed, opt_difficulty;
	newmenu_item m[16];
	...
	m[opt].min_value = m[opt].max_value = -1;
	...
	if (Last_secret_level < -1)
		sprintf(level_text+strlen(level_text)-1, ", 101-%d)", 100-Last_secret_level);
	else if (Last_secret_level == -1)
		sprintf(level_text+strlen(level_text)-1, ", 101)");

	Assert(strlen(level_text) < 32);

	m[opt].type = NM_TYPE_TEXT; m[opt].text = level_text; opt++;

	m[opt].type = NM_TYPE_INPUT; m[opt].text = slevel; m[opt].text_len=4; m[opt].min_value='0'; m[opt].max_value ='9'; opt++;
...
network_set_game_mode(int gamemode)
{
	...
	// only 4 starting points for co-op, don't mess with this
	if (Game_mode & GM_MULTI_ROBOTS)
		MaxNumNetPlayers = 4;
	else
		MaxNumNetPlayers = 8;
...
//		Players[i].net_killed_total = 0;
	...
	for (i=0; i<N_players; i++ )	{
		...
		{
			// this can happen if broadcast packets get bounced back to their source
		...
		if (Network_game_type == IPX_GAME) {
			const int zero = 0;
			if (memcmp(Netgame.players[i].network_info.ipx_info.server, &zero, 4))
...
//			mprintf((0, "Player %d starting in location %d\n" ,i ,np ));
	...
	for (i=0; i<N_players; i++ )	{
		...
		// Node spam! make sure this doesn't clog the socket's send buffer
		if (Network_game_type == IPX_GAME) {
...
network_select_teams(void)
{
	...
	// Here comes da menu
menu:
	m[0].type = NM_TYPE_TEXT; m[0].text = team_names[0]; m[0].text_len = CALLSIGN_LEN;
	...
	opt_team_b = opt;
	m[opt].type = NM_TYPE_TEXT; m[opt].text = team_names[1]; m[opt].text_len = CALLSIGN_LEN; opt++;
	for (i = 0; i < N_players; i++)
	{
		if (team_vector & (1 << i))
		{
...
network_select_players(void)
{
	...
	m[MAX_PLAYERS].type = NM_TYPE_TEXT;   m[MAX_PLAYERS].text = "";
	m[MAX_PLAYERS+1].type = NM_TYPE_MENU; m[MAX_PLAYERS+1].text = TXT_ACCEPT;

	m[0].value = 1;				// Assume server will play...

	sprintf( text[0], "%d. %-16s", 1, Players[Player_num].callsign );
	sprintf( title, "%s %d %s", TXT_TEAM_SELECT, MaxNumNetPlayers, "PLAYERS\nCHOOSE ACCEPT TO BEGIN.");

GetPlayersAgain:
	j=newmenu_do1( NULL, title, sizeof(m)/sizeof(m[0]), m, network_start_poll, 1 );
