...
//			return;
	...
	if (0 && obj == ConsoleObject) {
		// laser direction based on mouse cursor
		mouse_get_pos(&mouse_x, &mouse_y);
		gun_point.z = F1_0;
		gun_point.x = i2f(mouse_x-160)/240;

		if (Cockpit_mode == CM_FULL_SCREEN) {
			gun_point.y = i2f(mouse_y-100);
		} else if (Cockpit_mode == CM_FULL_COCKPIT) {
			gun_point.y = i2f(mouse_y-70);
		} else { // Cockpit_mode == CM_STATUS_BAR
			gun_point.y = i2f(mouse_y-75);
		}
		gun_point.y /= -280;

		vm_vec_rotate(&LaserDir,&gun_point,&m);
	} else
...
static const fix homing_turn_base[NDL] = {5, 6, 9, 10, 11};
