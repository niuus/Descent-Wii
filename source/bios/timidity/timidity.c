...
{
  FILE *fp;
  char tmp[1024], *w[MAXWORDS], *cp;  ToneBank *bank=0;
  ...
  if (!(fp=open_file(name, 1, OF_VERBOSE)))
   return -1;  while (fgets(tmp, sizeof(tmp), fp))  {
    line++;
    ...
    if (!strcmp(w[0], "dir"))    {      if (words < 2)       {        ctl->cmsg(CMSG_ERROR, VERB_NORMAL,          "%s: line %d: No directory given\n", name, line);        return -2;       }      for (i=1; i<words; i++)        add_to_pathlist(w[i]);    }  else if (!strcmp(w[0], "source"))
  {
    if (words < 2)
      {
        ctl->cmsg(CMSG_ERROR, VERB_NORMAL,
...
int Timidity_Init(int rate, int format, int channels, int samples)
{
  ...
  if ( (format&0xFF) == FORMAT_16 ) {
    play_mode->encoding |= PE_16BIT;
  }
  if ( (format&FORMAT_SIGNED) ) {
