...
static int sysex(uint32 len, uint8 *syschan, uint8 *sysa, uint8 *sysb, CFILE *rw)
{
  unsigned char *s=(unsigned char *)safe_malloc(len);
  int id, model, ch, port, adhi, adlo, cd, dta, dtb, dtc;
  if (len != (uint32)cfread(s, 1, len, rw))
    {
      free(s);
      return 0;
    }
...
static int dumpstring(int32 len, char *label)
{
  ...
  if (len != (int32)cfread(s, 1, len, rw))
    {
      free(s);
      return -1;
    }
...
static MidiEventList *read_midi_event(void)
{
  ...
  for (;;)
    {
      at+=getvl();
      if (cfread(&me, 1, 1, rw)!=1)
	{
	  ctl->cmsg(CMSG_ERROR, VERB_NORMAL, "%s: read_midi_event",
	       current_filename);
	  return 0;
	}
      ...
      else if(me==0xFF) /* Meta event */
	{
	  ...
	  if (type>0 && type<16)
		...
		if(len == 1)
		{
			cfread(&midi_port_number, 1, 1, rw);
...
static int read_track(int append)
{
  ...
  if ((cfread(tmp, 1, 4, rw) != 4) || (cfread(&len, 4, 1, rw) != 1))
    {
      ctl->cmsg(CMSG_ERROR, VERB_NORMAL,
...
MidiEvent *read_midi_file(CFILE *crw, int32 *count, int32 *sp)
{
  int32 len, divisions;
  int16 format, tracks;
  int i;
...
#ifdef HMP
  if (cfread(tmp, 1, 8, rw)!=8 || memcmp(tmp, "HMIMIDIP", 8))
  {
	  ctl->cmsg(CMSG_ERROR, VERB_NORMAL, "%s: Not a HMP MIDI file!", current_filename);
	  return 0;
  }
  format = 3;
  divisions = 192;
  cfseek(rw, 0x30, SEEK_SET);
  cfread(&tracks, 2, 1, rw);
  tracks = LE_SHORT(tracks) - 1;
  cfseek(rw, 0x30C, SEEK_SET);
  cfread(&len, 4, 1, rw);
  len = LE_LONG(len);
  cfseek(rw, len-8, SEEK_CUR);

#else
  int16_t divisions_tmp;
...
past_riff:
  ...
  cfread(&format, 2, 1, rw);
  cfread(&tracks, 2, 1, rw);
  cfread(&divisions_tmp, 2, 1, rw);
  ...
  if (len > 6)
    {
	  ...
	  cfseek(rw, len-6, SEEK_CUR); /* skip the excess */
    }
